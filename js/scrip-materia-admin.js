...
function obtenerListaBaseDatos(){
    ...
    $.post("./php/consultaListaMateriaUnifGrupo.php","datos",function(respuesta){
...
function cargarDatosPaginaTablaAula(numPagina){
    ...
    for (let index = aux ; index < (aux + maxFilasPagina) && index < datosTabla.length ; index++) {
        ...
        template += ` <tr>
                           <td>${index+1}</td>
                           <td class="codigosis-tabla">${element.codMateria}</td>
                           <td>${element.nombre}</td>
                           <td>${element.estudiantes}</td>
                           <td>${element.cantGrupos}</td>       
                     </tr>`;
    }
    $('#tabla-materia').html(template);
